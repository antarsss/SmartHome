async function setLightsData(listDevice) {
    ...
    if (listDevice.length == 0) {
        ...
        listDevice.forEach((data) => {
            ...
            var text = state == "ON" ? "style='color:green;'" : "style='color:red;'";
            ...
            lights += '<ul class="list-group borderless active" data-unit="wash-machine">' +
                ...
                '<img style="object-fit: contain; padding: 8px 8px 8px 0px" class="icons-state" src="/img/' + path + '">' +
                '<div><h5>' + data.deviceName + '</h5>' +
                '<h6 class="text-success" style="padding-left:5px">' + getPosition(data.position) + '</h6>' +
                '</div>' +
                '<p id="p' + data._id + '" class="ml-auto status" ' + text + 'style="padding: 16px 8px 8px 0px">' + state + '</p>' +
                '</li></ul> ';
        });
        await sleep(100);
